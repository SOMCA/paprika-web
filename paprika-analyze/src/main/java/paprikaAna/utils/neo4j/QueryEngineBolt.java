...
public class QueryEngineBolt {
	...
	public void getPropertyForAllApk(String nodeType, String property, String suffix) throws IOException {
		StatementResult result;
		try (Transaction tx = this.session.beginTransaction()) {
			String query = QueryEngineBolt.MATCHN + nodeType + ") RETURN n.app_key as app_key, n.name as name, n." + property
					+ " as " + property;
			result = tx.run(query);
	...
	public void resultToCSV(StatementResult result, String nameQuery) throws IOException {
		...
		try (Transaction tx = this.session.beginTransaction()) {
			...
			nodeQueryData.addParameter(PaprikaKeyWords.APPKEY, this.keyApp);
			 ...
			 */
			LowNode nodeQuery = new LowNode(PaprikaKeyWords.LABELQUERY);
			nodeQuery.addParameter(PaprikaKeyWords.APPKEY, this.keyApp);
			tx.run(this.graph.relation(nodeQuery, nodeFastRel,PaprikaKeyWords.REL_CODESMELLS_CAS));
			...
			while (result.hasNext()) {
				...
				if (val != null && !val.isNull()) {
					...
					LowNode nodeRel = new LowNode(PaprikaKeyWords.REL_CAS_CODE);
	...
	public void statsToCSV(Map<String, Double> stats, String nameQuery) throws IOException {
		...
		try (Transaction tx = this.session.beginTransaction()) {
			...
			// Je crée le noeud Query data, exemple: Blob
			LowNode nodeQueryData = new LowNode("QueryData");
			...
			LowNode node_QueryData;
			String labelQueryData = "Content";
			node_QueryData = new LowNode(labelQueryData);
			for (String key : keys) {
				node_QueryData.addParameter(key, stats.get(key));
			}
			node_QueryData.addParameter(Neo4JUser.NAMEATTRIBUTE, nameQuery + ":stats");
			node_QueryData.addParameter(Neo4JUser.APPKEY, this.keyApp);
			tx.run(this.graph.relationcreateRight(nodeQueryData, node_QueryData, "contains"));
			tx.success();
