...
public class SootAnalyzer extends Analyzer {
	...
	private int broadcastReceiverCount = 0;
	private int contentProviderCount = 0;
...
PaprikaAnalyzeMain.LOGGER.trace("G.reset();");
	...
	public void collectClassesMetrics() {
		...
		for (SootClass sootClass : sootClasses) {
			...
			if (!(name.equals(rClass) || name.startsWith(rsubClassStart) || name.equals(buildConfigClass)) && (!mainPackageOnly || name.startsWith(packs))) {
				collectClassMetrics(sootClass);
			}
		}
		// Now that all classes have been processed at least once (and the map
		// filled) we can process NOC
		for (SootClass sootClass : sootClasses) {
	...
	public void collectMethodsMetrics(SootMethod sootMethod) {
		...
		PaprikaMethod paprikaMethod = PaprikaMethod.createPaprikaMethod(sootMethod.getName(), modifiers,
		...
		if (sootMethod.hasActiveBody()) {
			...
			// Number of lines is the number of Units - number of Parameter - 1
			// (function name)
				...
				for (ValueBox valueBox : boxes) {
					...
					if (value instanceof FieldRef) {
						...
						if (field.declaringClass() == sootClass) {
							...
							// If we don't find the field it's inherited and
							// thus not used for LCOM2
			...
			if (isInit(sootMethod)) {
				...
				if (nbOfBranches == 1 && paprikaMethod.getUsedVariables().size() == 1
						&& sootMethod.getExceptions().isEmpty()) {
					...
					if (parameterCount == 1 && unitSize <= 4 && "void".equals(returnType)) {
				...
				if (externalTgtMethod == null) {
					...
					if (paprikaExternalClass == null) {
						paprikaExternalClass = PaprikaExternalClass
								.createPaprikaExternalClass(target.getDeclaringClass().getName(), paprikaApp);
					...
					for (Type type : target.getParameterTypes()) {
						...
						if (paprikaExternalArgument.getName() == "android.graphics.Bitmap$Config") {
							for (Unit unitChain : ((SootMethod) e.getSrc()).getActiveBody().getUnits()) {
								try {
									String nameOfStaticFieldRef = ((StaticFieldRef) ((GRValueBox) ((GAssignStmt) unitChain)
											.getRightOpBox()).getValue()).getFieldRef().name();
									if ("ARGB_8888".equals(nameOfStaticFieldRef)) {
										argb8888Count++;
	...
	public void collectClassMetrics(SootClass sootClass) {
		...
		for (SootField sootField : sootClass.getFields()) {
			...
			PaprikaVariable paprikaVariable = PaprikaVariable.createPaprikaVariable(sootField.getName(),
					sootField.getType().toString(), modifier, paprikaClass);
			varCount++;
	...
	private boolean isInnerClassStatic(SootClass innerClass) {
		for (SootField sootField : innerClass.getFields()) {
			// we are looking if the field for non static inner class generated
			// during the compilation (with the convention name) exists
			if ("this$0".equals(sootField.getName())) {
	...
	/**
	 * Test if a class contains a method with same name, parameters and return
	 * type
	 * 
	 * @param sootClass
	 * @param sootMethod
	...
	private boolean classContainsMethod(SootClass sootClass, SootMethod sootMethod) {
		// Here unsafe just means it will return null (instead of throwing an
		// exception).
		if (sootClass.getMethodUnsafe(sootMethod.getName(), sootMethod.getParameterTypes(),
				sootMethod.getReturnType()) != null)
			return true;
		else
			return false;
	}
