...
/**
 * PaprikaTimer is a Timer who launch the method run each X time. The time is
 * give per the PaprikaWebMain. He launch container of the queue and remove
 * stopped container.
 * 
 * @author guillaume
 *
...
public class PaprikaTimer extends TimerTask {
	...
	private int num = 0;
	private int cooldownUpdate=5;

	/**
	 * 
	 * @param containerRun
	 *            The container currently run(if server restart)
	 */
	...
	public void run() {
		...
		System.out.flush();
		num++;
		DataSaveFunctions dataFct = new DataSaveFunctions();
		...
		for (int i = 0; i < containerRun.length; i++) {
			...
			if (container == null) {
				...
				if (needUpdate) {
					...
					dataFct.runCommand("MATCH(n:DataSave) SET n.containerRun" + i + "=\"" + this.containerRun[i] + "\"");
		...
		if (PaprikaWebMain.getVersionOnAnalyze() < this.parallelanalyzeMax) {
			...
			if (otherStrContainerConfig != null) {
				...
				try {
					...
					newid = creation.id();
					facade.setParameterOnNode(
							otherStrContainerConfig[otherStrContainerConfig.length - (githubok ? 2 : 1)], "idContainer",
							newid);
